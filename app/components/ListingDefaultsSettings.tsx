...
export default function ListingDefaultsSettings() {
  ...
  const [handlingDays, setHandlingDays] = useState("7");
  ...
  const save = () => {
    const hd = Math.min(30, Math.max(1, Math.floor(Number(handlingDays) || 7)));
    writeListingDefaults({ bestOffer, handlingDays: hd });
