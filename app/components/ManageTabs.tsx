...
import { Heart, ShoppingBag, Tag, Archive, CircleDollarSign } from "lucide-react";

// 「商品管理」の5サブタブ：お気に入り → 仕入れ商品 → 出品中 → 売れた商品 → 終了商品（この順）。件数つき。
// 1つのページ(/manage)を ?tab= で切替。売れた商品＝eBayで売却済み（出品中タブから分離）。終了商品＝停止中／過去の出品。
const TABS = [
  { key: "fav", label: "お気に入り", Icon: Heart },
  { key: "bought", label: "仕入れ商品", Icon: ShoppingBag },
  { key: "listed", label: "出品中", Icon: Tag },
  { key: "sold", label: "売れた商品", Icon: CircleDollarSign },
