...
export default function CopyText({ text, label = "コピー" }: { text: string; label?: string }) {
  ...
  return (
    <div className="relative bg-[#F5F7FA] border border-gray-100 rounded-xl p-3.5">
      <pre className="whitespace-pre-wrap break-words text-[12px] text-gray-700 leading-relaxed font-sans pr-16">{text}</pre>
      <button
        type="button"
        onClick={copy}
        className="absolute top-2.5 right-2.5 inline-flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-[11px] font-bold text-gray-600 active:bg-gray-50"
      >
