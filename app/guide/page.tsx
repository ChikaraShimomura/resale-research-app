...
const STEPS: Step[] = [
  ...
  },
  {
    ...
    Icon: PenLine,
    title: "eBayに出品する",
    desc: "まず商品カードの赤い「楽天で仕入れる」ボタンを押すと「eBay自動出品」が解放されます（商品画像をタップしても解放されません）。あとはボタンを押すだけ。タイトル・価格・カテゴリは自動で入り、楽天の画像で出品できます。",
    tips: ["「eBay自動出品」が灰色なら、先に赤い「楽天で仕入れる」を押すと解放", "出品の最後に「先に楽天で注文しました」のチェックが必要", "カテゴリが赤字で出たら、タイトルを具体的にして開き直すと通る"],
  ...
  },
  {
    ...
    Icon: Package,
    title: "売れたら発送する",
    desc: "落札後は日本郵便の国際郵便で発送します。小型・軽量は「国際エアパケット」（旧・国際eパケットライト／追跡あり・2kgまで）、高額や大きめは「EMS」（追跡＋補償）が定番。海外宛ては内容品を英語で電子申告するため、送り状は「国際郵便マイページ」で作成します（2024年から全世界宛てで電子データの事前提出が必須・手書きラベルは原則使えません）。",
    tips: ["小型は国際エアパケット／高額・大型はEMS（どちらも追跡あり）", "マイページで送り状＋内容品を英語申告（品名・数量・価格・HSコード）", "追跡番号は必ずeBayの注文に登録（未登録だと売上保留・未着クレームの原因）"],
