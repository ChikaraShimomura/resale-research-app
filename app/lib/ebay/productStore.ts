...
// 配信集計(getUsedCatalog/getBoughtItems/getFavoriteItems)と定義を必ず揃える。楽天モデルは applyDisplayProfit 側で扱うので触らない。
function normalizeUsedRoi(p: ProfitProduct): ProfitProduct {
  const buy = p.source?.price ?? 0;
  const site = String(p.source?.site ?? ""); // 型は "rakuten" 固定だが中古ビルドは "hardoff"/"2ndstreet" を入れる
