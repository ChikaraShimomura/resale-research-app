      ...
      <main className="max-w-2xl mx-auto p-3">
        <article className="bg-white rounded-2xl border border-[#A98B5C]/25 shadow-sm p-4">
          ...
          <Section title="第1条（本サービスの内容）">
            <p>本サービスは、楽天市場等の商品情報をもとに、海外（eBay等）での想定相場・想定利益を参考表示し、出品作業を補助するツールです。</p>
          ...
          <Section title="第2条（アカウント登録）">
            <Bullets items={[
              "利用には会員登録（メールアドレス等）が必要。登録情報は正確・最新に保つこと。",
              "アカウントの管理責任は利用者にあり、第三者への貸与・共有は不可。",
              "登録情報の取り扱いは、別途定めるプライバシーポリシーによる。",
            ]} />
          </Section>
          ...
          <Section title="第4条（利用者の責任）">
            ...
            <Bullets items={[
              "eBay・楽天・Payoneer等、利用する各サービスの利用規約・ポリシーを遵守すること。違反による利用停止・アカウント閉鎖等の責任は利用者が負う。",
              "出品・販売・輸出する商品が、各プラットフォーム規約および関係法令（関税法・外為法・輸出規制、薬機法、その他の各種規制）に適合していること。危険物・輸出禁制品・各国の規制品等の発送可否は利用者が確認すること。",
              "中古品を反復継続して仕入れ・転売する場合、古物営業法上の古物商許可が必要となることがあります。要否は利用者自身で確認すること（新品の転売には原則不要）。",
              "在庫を持たずに出品する形態（無在庫）は、各プラットフォーム規約で制限・禁止される場合があります。採用の可否・リスクは利用者の責任で判断すること。",
              "出品に用いる商品画像・文章等の知的財産権・利用許諾を確認すること。可能な限り、利用者自身が撮影した実物写真の使用を推奨。",
          ...
          <Section title="第6条（免責）">
            <Bullets items={[
              "本サービスが表示する相場・利益・価格・カテゴリ等は想定・目安であり、正確性・実現性・特定目的への適合性を保証しません。",
              "本サービスの利用または利用できなかったことにより利用者に生じた損害について、責任を負いません。ただし本サービスの故意または重大な過失による場合を除きます。",
              "外部サービス（eBay・楽天・決済事業者等）の仕様変更・停止・障害により生じた不利益について、責任を負いません。",
            ]} />
