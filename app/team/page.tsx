...
import { getRoster, getPending, getMyTeams } from "../lib/team";
...
export default async function TeamPage() {
  ...
  const [roster, pending, myTeams] = actor
    ? await Promise.all([getRoster(actor), getPending(actor), getMyTeams(actor)])
    : [[], [], []];
  ...
  return (
    <div className="min-h-dvh bg-[#F5F7FA] pb-nav">
      ...
      <main className="max-w-2xl mx-auto px-4 py-5">
        ...
        {actor && email ? (
          <TeamManager roster={roster} pending={pending} myTeams={myTeams} />
        ) : (
