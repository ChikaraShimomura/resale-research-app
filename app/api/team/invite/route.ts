...
import { createInvite, hasPerm, getTeamName, getMyTeams } from "../../../lib/team";
...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => ({}))) as { email?: string; ownerActor?: string };
  const email = (body.email || "").trim().toLowerCase();
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 200) {
    return Response.json({ ok: false, error: "メールアドレスを正しく入力してください。" }, { status: 400 });
  }

  // 委譲：別オーナーのチームに招待する場合は、そのオーナーで manage 権限を持つ会員だけ許可（昇格防止）。
  //   通る場合は招待先＝そのオーナーのチーム（団体名義もオーナーのメール）。省略/自分なら従来どおり自分のチーム。
  let teamOwnerActor = actor;
  let teamOwnerEmail = ownerEmail;
  const reqOwner = (body.ownerActor || "").trim();
  if (reqOwner && reqOwner !== actor) {
    if (!(await hasPerm(reqOwner, actor, "manage"))) {
      return Response.json({ ok: false, error: "このチームを管理する権限がありません。" }, { status: 403 });
    }
    teamOwnerActor = reqOwner;
    // 招待文面に出すオーナーのメール。逆引き（参加チーム）→チーム名→actor の順でフォールバック。
    teamOwnerEmail =
      (await getMyTeams(actor)).find((t) => t.ownerActor === reqOwner)?.ownerEmail ||
      (await getTeamName(reqOwner)) ||
      reqOwner;
  }
  if (email === teamOwnerEmail.toLowerCase()) {
    return Response.json({ ok: false, error: "オーナー本人は招待できません。" }, { status: 400 });
  }
  ...
  // 同一メールへの再招待は新トークンで上書き＝最新リンクのみ有効。委譲時はオーナーのチームに入れる。
  const token = await createInvite(teamOwnerActor, teamOwnerEmail, email);
  ...
  <p><b>${teamOwnerEmail}</b> さんから、輸出ラボの<b>チーム共有</b>に招待されました。</p>
  <p>承認すると、${teamOwnerEmail} さんの<b>「仕入れた商品」一覧</b>と<b>収支（仕入れ額・売上額）</b>を見られるようになります。</p>
