...
import { listLiveDeals, removeDeal, recordSold, USD_JPY } from "../../../lib/ebay/stats";
...
export async function GET() {
  ...
  if (!actor) return Response.json({ ok: false, deals: [] });
  const deals = await listLiveDeals(actor);
  return Response.json({ ok: true, deals }, { headers: { "Cache-Control": "private, no-store" } });
}
