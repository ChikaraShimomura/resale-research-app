...
async function main() {
  ...
  async function processEbayItem(ebayItem) {
    ...
    for (const rakutenItem of rakutenItems.slice(0, 10)) { // 候補を5→10に拡大（同一品の成立率=歩留まり向上）
      ...
      // 落札seedは価格(=落札額)が確定→現金純利益率(ポイント除外＋着地コスト後＝配信 displayProfit と一致)で床を当てる。
      // 現金<1%(0・マイナス)＝論外を画像マッチ前に落とす(Anthropic節約＋死蔵防止)。1〜9%=ポイ活専用/≥10%=現金で稼げる は配信側 applyDisplayProfit が区別。
      if (ebayItem.soldSeed) {
        ...
        if (netRate < 1) continue; // 現金純利益率<1%(0・マイナス)は論外
      }
  ...
  // 現金純利益率<1%(0・マイナス)＝論外の商品はカタログから外す。
  // 表示で隠すのでなく、カタログ自体から外す＝sitemap/商品ページ/topProducts含め一切 利益商品扱いしない。
  // ※現金1〜9%は残す＝「ポイ活専用」として掲載(種別は配信 applyDisplayProfit が付与)。現金で稼げる(≥10%)/ポイ活の区別は配信側で。
  // restored(運営が手動復活)は裁量で常に残す（配信 /api/products と同じ扱い）。
  {
    const before = profitableProducts.length;
    const kept = profitableProducts.filter(p => p.restored || netProfitRate(p) >= 1);
