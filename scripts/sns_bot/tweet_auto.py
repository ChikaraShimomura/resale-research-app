...
# ── 共通: 読みやすさ(スマホで読ませる。こまめに改行して短い行のまとまりで見せる) ──
READABILITY = (
    "読みやすさ最重要。スマホでスッと読めるように“こまめに改行”して、短い行のまとまりで見せる（今どきのXの読ませ方）。"
    "文の切れ目・話題の切れ目でどんどん改行する。だいたい1〜2文ごとに改行を入れ、意味のまとまりの区切りには空行を1つ挟んでブロックに分けてよい。"
    "1行は短めに（長い一文をダラダラ続けない）。文には句読点(、。)をきちんと打つ。"
    "ただし単語ごとのブツ切り・意味のない空行の連発はしない（目的は“読みやすさ”で、スカスカにするのが目的ではない）。"
    "改行を増やしても、ラフでカジュアルな話し言葉のテンポは保つ。"
)
