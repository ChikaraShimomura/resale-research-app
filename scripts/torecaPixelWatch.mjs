...
const STUCK_MIN = 300; // 見守りは4時間で起動し直す。それでも5時間進んでいなければ固まったまま
const REMIND_H = 12;
...
export function judge(hbRaw, nowMs) {
  ...
  if (!hb || !Number.isFinite(Number(hb.at))) return { state: "down", reason: "合図が読めない", hb: null, hbAgeMin: Infinity, loopAgeMin: null };
  ...
  if (hbAgeMin > DOWN_MIN) return { state: "down", reason: `最後の合図から ${dur(hbAgeMin)}`, hb, hbAgeMin, loopAgeMin };
  if (loopAgeMin != null && loopAgeMin > STUCK_MIN) return { state: "stuck", reason: `ワーカーのループが ${dur(loopAgeMin)} 進んでいない`, hb, hbAgeMin, loopAgeMin };
  return { state: "ok", reason: "", hb, hbAgeMin, loopAgeMin };
}
...
export function decide(prev, cur, nowMs) {
  ...
  if (bad(cur.state)) {
    if (!prev || !bad(prev.state)) return { mail: "alert", next: { state: cur.state, since: nowMs, lastMailAt: nowMs } };
    if (prev.state !== cur.state) return { mail: "alert", next: { ...prev, state: cur.state, lastMailAt: nowMs } };
...
<p style="color:#666;font-size:12px">様子: <a href="${STATUS_URL}">pixel-status.md</a></p>`,
  ...
  const isStuck = cur.state === "stuck";
  const head = kind === "remind" ? "🔴 (続報) " : "🔴 ";
  return {
    subject: isStuck
      ? `${head}Pixel のワーカーが固まっています (${cur.reason})`
      : `${head}Pixel が止まりました (最後の合図 ${last})`,
...
<p>${
      isStuck
        ? "Pixel の中の見守りが自動で起動し直していますが、まだ戻っていません。"
        : "Pixel ごと止まっているか、通信が切れている可能性が高いです。<b>Pixel の電源・Wi-Fi・充電</b>を確かめてください (電源が入っていれば、再起動するだけで自動で動き出します)。"
    } 戻ったらもう一度メールでお知らせします。</p>
...
async function main() {
  ...
  const { mail, next } = decide(prev, cur, now);
  if (mail) {
  ...
  if (JSON.stringify(next) !== JSON.stringify(prev)) await kv(["SET", KV_KEY, JSON.stringify(next)]);
}

if (process.argv[1] && /torecaPixelWatch\.mjs$/.test(process.argv[1])) {
  main().catch((e) => {
    console.error(`torecaPixelWatch: ${e?.message || e}`);
    process.exit(1);
  });
}
