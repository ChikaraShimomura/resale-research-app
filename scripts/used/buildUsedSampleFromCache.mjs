...
// ※「ボールペン」の単独除外は撤回(2026-07-09)：モンブラン等の高級ボールペンを巻き込むため。安物ペンは具体名(エナージェル/ジェットストリーム/ぺんてる/シャーペン)＋中央¥6000ゲートで弾く。
const EXCLUDE = /資生堂|キャンメイク|DHC|ルルルン|セザンヌ|KATE|ファンデ|チーク|アイシャドウ|クレンジング|フェイスマスク|眉ペンシル|コスメ|リップ|エナージェル|ジェットストリーム|ぺんてる|シャーペン|ノック式|食品|お菓子|レトルト|カード|ポケカ|MTG(?![-\w])|ヴァンガード|遊戯王|テラスタル|デュエ|バトスピ|ユニオンアリーナ|ヴァイス|ビルディバイド|シャドウバース|ジュエリー|ネックレス|指輪|ブレスレット|ピアス|イヤリング|貴金属/i; // 末尾のジュエリー系=2026-07-16対象外化(ユーザー指示)。ブランドトークン(グッチ等)経由の通過や種プール残存カテゴリも確実に弾く最終ゲート。
...
function genreOf(category) {
  ...
  // ブランド品(2026-07-07)。時計は上の腕時計バケツで拾う（海外ファッション時計も name に「腕時計」を含む）。ここはバッグ/財布。
  // ※ジュエリーは2026-07-16に対象から除外(ユーザー指示)＝クエリ/BRAND_USED_KWごと撤去済み(バケツも削除)。
  if (/バッグ|ハンドバッグ|トートバッグ|ショルダーバッグ|ボストンバッグ|財布|長財布|ウォレット/i.test(c)) return "バッグ";
  // ブランド サングラス/眼鏡・高級筆記具(2026-07-09追加・画像レール向き)。
  if (/サングラス|メガネ|眼鏡|アイウェア|sunglasses|eyewear|レイバン|オークリー|トムフォード|ペルソール|persol|oakley|ray-?ban|白山眼鏡/i.test(c)) return "メガネ";
  if (/万年筆|ボールペン|筆記具|fountain pen|ballpoint|モンブラン|ペリカン|ナミキ|ウォーターマン|montblanc|pelikan|waterman/i.test(c)) return "筆記具";
  return "中古";
}
